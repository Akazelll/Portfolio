...
const ProjectPage = ({ projects }: { projects: Project[] }) => {
  ...
  // Fungsi untuk memotong deskripsi tetap di sini
  const truncateDescription = (text: string, length: number = 100) => {
    return text.length > length ? text.substring(0, length) + "..." : text;
  };

  // Varian animasi untuk container kartu proyek
  const containerVariants = {
    hidden: { opacity: 0 },
    visible: {
      opacity: 1,
      transition: {
        staggerChildren: 0.15, // Sedikit percepat stagger
      },
    },
  };
  ...
  return (
    // Gunakan <section> karena ini adalah bagian utama dari halaman Anda
    <section id="projects" className="w-full bg-gray-100 py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <motion.div
          ...
          initial="hidden"
          whileInView="visible" // Animasi dimulai saat kontainer terlihat
              ...
              <Link href={`/projects/${project.id}`} legacyBehavior>
                <a className="flex h-full flex-col">
                  <div className="relative h-48 w-full">
                    {/* PERBAIKAN: Gunakan project.image dan tambahkan placeholder */}
                    {project.image ? (
                       <Image
                        src={project.image}
                        alt={project.title}
